...
(function () {
  ...
  document.querySelectorAll('.v3-rating').forEach(function (el) {
    ...
    for (var j = 0; j < empty; j++) html += '<i class="fa-regular fa-star star-empty"></i>';
  ...
  var prevBtn = document.querySelector('.v3-prev');
  var nextBtn = document.querySelector('.v3-next');

  function v3StepSlide(direction) {
    if (v3Swiper.autoplay && v3Swiper.autoplay.running) {
      v3Swiper.autoplay.stop();
    }
    v3Swiper.params.speed = 500;
    if (direction === 'next') {
      v3Swiper.slideNext();
    } else {
      v3Swiper.slidePrev();
    }
    clearTimeout(v3Swiper._resumeTimer);
    v3Swiper._resumeTimer = setTimeout(function () {
      v3Swiper.params.speed = 4000;
      v3Swiper.autoplay.start();
    }, 1200);
  }
